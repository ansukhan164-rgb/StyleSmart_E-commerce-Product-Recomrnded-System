...
__export(index_exports, {
  AGENT_MODE_DEFAULTS: () => AGENT_MODE_DEFAULTS,
  ...
  MODEL_DOLLAR_COSTS: () => MODEL_DOLLAR_COSTS,
  OPENROUTER_MODEL_PREFIX: () => OPENROUTER_MODEL_PREFIX,
  Semaphore: () => Semaphore,
  WriteMutex: () => WriteMutex,
  autoTierIds: () => autoTierIds,
  autoTierProviderHints: () => autoTierProviderHints,
  buildAutoTierMap: () => buildAutoTierMap,
  buildModelTierMap: () => buildModelTierMap,
  buildTextResponse: () => buildTextResponse,
  buildToolUseResponse: () => buildToolUseResponse,
  calculateDollarCost: () => calculateDollarCost,
  classifySpawnTask: () => classifySpawnTask,
  configureAIProxy: () => configureAIProxy,
  countToolCalls: () => countToolCalls,
  createMockStreamFn: () => createMockStreamFn,
  ...
  fingerprintMessages: () => fingerprintMessages,
  ...
  getAgentModeOverrides: () => getAgentModeOverrides,
  getAutoTierOverrides: () => getAutoTierOverrides,
  getAvailableModels: () => getAvailableModels,
  getImageModelEntry: () => getImageModelEntry,
  getMaxOutputTokens: () => getMaxOutputTokens,
  getModelBillingModel: () => getModelBillingModel,
  getModelDisplayName: () => getModelDisplayName,
  getModelEntry: () => getModelEntry,
  getModelFamily: () => getModelFamily,
  getModelShortDisplayName: () => getModelShortDisplayName,
  getModelTier: () => getModelTier,
  getModelsByProvider: () => getModelsByProvider,
  getProviderLabel: () => getProviderLabel,
  getRoutingConfig: () => getRoutingConfig,
  getSubagentOrchestrationReliability: () => getSubagentOrchestrationReliability,
  inferProviderFromModel: () => inferProviderFromModel,
  isAutoModel: () => isAutoModel,
  isConcurrencySafe: () => isConcurrencySafe,
  isOpenRouterModel: () => isOpenRouterModel,
  loadAllHooks: () => loadAllHooks,
  loadHooksFromDir: () => loadHooksFromDir,
  microcompact: () => microcompact,
  partitionToolCalls: () => partitionToolCalls,
  resolveAgentModeDefault: () => resolveAgentModeDefault,
  resolveApiKey: () => resolveApiKey,
  resolveModel: () => resolveModel,
  resolveModelId: () => resolveModelId,
  runAgentLoop: () => runAgentLoop,
  selectModelForSpawn: () => selectModelForSpawn,
  sendMessage: () => sendMessage,
  sendMessageJSON: () => sendMessageJSON,
  sendMessages: () => sendMessages,
  setAgentModeOverrides: () => setAgentModeOverrides,
  setAutoTierOverrides: () => setAutoTierOverrides,
  setRoutingConfig: () => setRoutingConfig,
  shortHash: () => shortHash,
  stripOpenRouterPrefix: () => stripOpenRouterPrefix,
  sumUsage: () => sumUsage,
...
function getImageModelEntry(id) {
  if (id in IMAGE_MODEL_CATALOG) {
    return IMAGE_MODEL_CATALOG[id];
  }
...
function getModelShortDisplayName(id) {
  if (!id) return "Unknown";
  if (id === AUTO_MODEL_ID) return "Auto";
  const entry = getModelEntry(id);
...
function inferProviderFromModel(modelId, fallback = "anthropic") {
...
function getModelBillingModel(id) {
  const entry = getModelEntry(id);
...
var MODEL_DOLLAR_COSTS = {
  "gpt-5.4-nano": { inputPerMillion: 0.2, cacheWritePerMillion: 0.25, cachedInputPerMillion: 0.02, outputPerMillion: 1.25 },
  haiku: { inputPerMillion: 1, cacheWritePerMillion: 1.25, cachedInputPerMillion: 0.1, outputPerMillion: 5 },
  "gpt-5.4-mini": { inputPerMillion: 0.75, cacheWritePerMillion: 0.9375, cachedInputPerMillion: 0.075, outputPerMillion: 4.4 },
  sonnet: { inputPerMillion: 3, cacheWritePerMillion: 3.75, cachedInputPerMillion: 0.3, outputPerMillion: 15 },
  opus: { inputPerMillion: 5, cacheWritePerMillion: 6.25, cachedInputPerMillion: 0.5, outputPerMillion: 25 }
};
function calculateDollarCost(modelOrAgentMode, inputTokens, outputTokens, cachedInputTokens = 0, cacheWriteTokens = 0) {
  ...
  return inputTokens * costs.inputPerMillion / 1e6 + cacheWriteTokens * costs.cacheWritePerMillion / 1e6 + cachedInputTokens * costs.cachedInputPerMillion / 1e6 + outputTokens * costs.outputPerMillion / 1e6;
}
function getSubagentOrchestrationReliability(id) {
...
function getAvailableModels(filter = { generation: "current" }) {
...
function createMockStreamFn(responses, onCall) {
  ...
  return (_model, context, options) => {
    ...
    if (signal?.aborted) {
      ...
      const stream2 = (0, import_pi_ai.createAssistantMessageEventStream)();
      queueMicrotask(() => {
...
function approxMessageChars(msg) {
  if (msg.role === "user") return extractUserText2(msg).length;
  if (msg.role === "assistant") {
    let total = 0;
    for (const block of msg.content) {
...
function parseHookMetadata(hookDir) {
  const hookMdPath = (0, import_path.join)(hookDir, "HOOK.md");
  if (!(0, import_fs.existsSync)(hookMdPath)) return null;
  try {
    const raw = (0, import_fs.readFileSync)(hookMdPath, "utf-8");
...
async function loadHandlerFromDir(hookDir) {
  ...
  for (const ext of HANDLER_EXTENSIONS) {
    const candidate = (0, import_path.join)(hookDir, `handler${ext}`);
...
async function loadHooksFromDir(dir) {
  ...
  for (const entry of entries) {
    const hookDir = (0, import_path.join)(dir, entry);
    if (!(0, import_fs.statSync)(hookDir).isDirectory()) continue;
...
async function loadAllHooks(workspaceDir) {
  ...
  const bundledDir = (0, import_path.resolve)(moduleDir, "bundled");
  const workspaceHooksDir = (0, import_path.join)(workspaceDir, "hooks");
