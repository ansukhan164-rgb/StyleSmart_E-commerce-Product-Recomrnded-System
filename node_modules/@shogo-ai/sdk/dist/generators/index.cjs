...
__export(generators_exports, {
  CUSTOM_REGION_END: () => CUSTOM_REGION_END,
  CUSTOM_REGION_START: () => CUSTOM_REGION_START,
  CUSTOM_ROUTES_SCAFFOLD: () => CUSTOM_ROUTES_SCAFFOLD,
  DEFAULT_PRISMA_CONFIG: () => DEFAULT_PRISMA_CONFIG,
  DEFAULT_PRISMA_HEADER: () => DEFAULT_PRISMA_HEADER,
  FEATURE_DEPS: () => FEATURE_DEPS,
  GENERATED_FILE_LICENSE_HEADER: () => GENERATED_FILE_LICENSE_HEADER,
  PRISMA_CONFIG_MANAGED_COMMENT: () => PRISMA_CONFIG_MANAGED_COMMENT,
  SCHEMA_HEADER_MANAGED_COMMENT: () => SCHEMA_HEADER_MANAGED_COMMENT,
  SCHEMA_HEADER_REGION_ID: () => SCHEMA_HEADER_REGION_ID,
  configIsDowngraded: () => configIsDowngraded,
  enforcePrismaConfig: () => enforcePrismaConfig,
  enforceSchemaHeader: () => enforceSchemaHeader,
  ensureCustomRoutes: () => ensureCustomRoutes,
  ensureFeatureDeps: () => ensureFeatureDeps,
  extractProtectedRegions: () => extractProtectedRegions,
  generateAdminRoutes: () => generateAdminRoutes,
  generateApiClient: () => generateApiClient,
  generateApiDomainStore: () => generateApiDomainStore,
  generateApiOverview: () => generateApiOverview,
  generateAuthStore: () => generateAuthStore,
  generateDbModule: () => generateDbModule,
  generateDocs: () => generateDocs,
  generateDocsSiteScaffold: () => generateDocsSiteScaffold,
  generateDocsTsConfig: () => generateDocsTsConfig,
  generateDomainStore: () => generateDomainStore,
  generateFromPrisma: () => generateFromPrisma,
  generateMSTCollection: () => generateMSTCollection,
  generateMSTCollections: () => generateMSTCollections,
  generateMSTDomain: () => generateMSTDomain,
  generateMSTModel: () => generateMSTModel,
  generateMSTModels: () => generateMSTModels,
  generateModelDoc: () => generateModelDoc,
  generateModelHooks: () => generateModelHooks,
  generateModelRoutes: () => generateModelRoutes,
  generateModelStore: () => generateModelStore,
  generateModelTypes: () => generateModelTypes,
  generateModelsIndex: () => generateModelsIndex,
  generateRoutes: () => generateRoutes,
  generateRoutesIndex: () => generateRoutesIndex,
  generateServer: () => generateServer,
  generateServerFunctions: () => generateServerFunctions,
  generateShogoClient: () => generateShogoClient,
  generateSqliteDbModule: () => generateSqliteDbModule,
  generateStores: () => generateStores,
  generateStoresIndex: () => generateStoresIndex,
  generateTypes: () => generateTypes,
  generateTypesIndex: () => generateTypesIndex,
  generateTypesPerModel: () => generateTypesPerModel,
  generateVoiceComponents: () => generateVoiceComponents,
  getIdField: () => getIdField,
  getRelationFields: () => getRelationFields2,
  getScalarFields: () => getScalarFields,
  getUserModel: () => getUserModel,
  hasMarkedSchemaHeader: () => hasMarkedSchemaHeader,
  hasUserModel: () => hasUserModel,
  headerIsDowngraded: () => headerIsDowngraded,
  mergeProtectedRegions: () => mergeProtectedRegions,
  parsePrismaSchema: () => parsePrismaSchema,
...
function ensureCustomRoutes(projectDir) {
  const tsPath = (0, import_path.join)(projectDir, "custom-routes.ts");
  const tsxPath = (0, import_path.join)(projectDir, "custom-routes.tsx");
  if ((0, import_fs.existsSync)(tsPath)) {
    return { path: "./custom-routes", created: false, absolutePath: tsPath };
  }
  if ((0, import_fs.existsSync)(tsxPath)) {
    return { path: "./custom-routes", created: false, absolutePath: tsxPath };
  }
  (0, import_fs.writeFileSync)(tsPath, CUSTOM_ROUTES_SCAFFOLD, "utf-8");
...
function ensureFeatureDeps(opts) {
  const packageJsonPath = (0, import_path2.resolve)(opts.cwd, "package.json");
  const missing = {};
  const added = {};
  const warnings = [];
  ...
  let pkg;
  try {
    raw = (0, import_fs2.readFileSync)(packageJsonPath, "utf-8");
    pkg = JSON.parse(raw);
  } catch (err) {
    warnings.push(
  ...
  if (modified && !opts.dryRun) {
    ...
    (0, import_fs2.writeFileSync)(packageJsonPath, serialized);
  }
