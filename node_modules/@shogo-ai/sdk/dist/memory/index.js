...
${line}`, "utf-8");
  ...
  addDaily(entry, date) {
    ...
    const path = join2(this.workspaceDir, "memory", `${d}.md`);
...
${line}`, "utf-8");
  ...
  readMemoryBullets() {
    const memoryMd = join2(this.workspaceDir, "MEMORY.md");
    if (!existsSync2(memoryMd)) return [];
    const raw = readFileSync2(memoryMd, "utf-8");
    const bullets = [];
...
${lines.join("\n")}
`;
    const file = join2(this.workspaceDir, "MEMORY.md");
    const tmp = `${file}.tmp`;
